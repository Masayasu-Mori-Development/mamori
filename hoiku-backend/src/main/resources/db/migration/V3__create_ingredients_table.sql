-- 食材レベル ENUM
CREATE TYPE ingredient_level AS ENUM ('system', 'organization', 'facility');
...
COMMENT ON TABLE hoiku.ingredients IS '食材マスタ（3階層: system → organization → facility）';
COMMENT ON COLUMN hoiku.ingredients.parent_ingredient_id IS '親食材ID（継承元）';
COMMENT ON COLUMN hoiku.ingredients.waste_rate IS '廃棄率（%）';
COMMENT ON COLUMN hoiku.ingredients.allergens IS 'アレルゲン一覧（配列）';
...
('system', 'SYS-001', '精白米', 'セイハクマイ', '穀類', 'g', 0,
    358, 6.1, 0.9, 77.6, 5, 0.8, ARRAY[]::TEXT[]),
('system', 'SYS-002', '鶏卵', 'ケイラン', '卵類', 'g', 13,
    151, 12.3, 10.3, 0.3, 51, 1.8, ARRAY['卵']::TEXT[]),
('system', 'SYS-003', '牛乳', 'ギュウニュウ', '乳類', 'ml', 0,
    67, 3.3, 3.8, 4.8, 110, 0.02, ARRAY['乳']::TEXT[]),
('system', 'SYS-004', 'にんじん', 'ニンジン', '野菜類', 'g', 10,
    37, 0.6, 0.1, 9.3, 28, 0.2, ARRAY[]::TEXT[]),
('system', 'SYS-005', '小麦粉（薄力粉）', 'コムギコ', '穀類', 'g', 0,
    368, 8.0, 1.7, 75.9, 17, 0.6, ARRAY['小麦']::TEXT[]);
